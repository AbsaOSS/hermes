...
class PluginDefinitionsTest extends FunSuite {
  ...
  test("testGetPluginNames") {
    ...
    val expectedPlugins = Set("BashPlugin", "InfoFileComparison", "DatasetComparison")
    assert((expectedPlugins -- plugins).isEmpty)
  }

  test("testGetPlugin") {
    val plugin = pluginDefinitions.getPlugin("BashPlugin")
    assert(plugin.name == "BashPlugin")
    assert(plugin.isInstanceOf[BashPlugin])
  }
